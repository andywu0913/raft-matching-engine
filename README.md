...
A POC putting a limit-order matching engine behind [hashicorp/raft] so the engine itself is HA — every acked order survives a single-node loss, and a follower takes over as leader in seconds. Using `hashicorp/raft` means no custom WAL, log truncation, snapshot/restore, or replication. The FSM only defines `Apply`/`Snapshot`/`Restore` and the library handles the rest.

> **Note:** real matching engines don't do this. Quorum-commit per order adds milliseconds of latency, so exchanges use single-leader engines with custom WAL shipping. Throughput here (~3k QPS local) is bounded by raft commit, not the engine.
...
- **Snapshot/Restore**: protobuf-encoded snapshot of books + dedup + `next_order_id` + `applied_index`. Snapshot deep-copies on the FSM goroutine, then the slow `Persist` runs lock-free on the frozen view.
...
If the original cluster is lost (every node's hardware is gone) but you have a backup of one node's data dir, you can bring up a new single-node cluster from it:

```bash
# 1. Copy the surviving node's data dir onto the new machine. The
#    subdirectory name under --data-dir must match the new node-id:
scp -r olduser@old-host:/path/data/node1 newuser@new-host:/path/data/phoenix

# 2. On the new machine, start with --recover. raft.RecoverCluster rewrites
